...
const COMMIT_TYPES: CommitType[] = ['feat', 'fix', 'refactor', 'docs', 'chore', 'style'];

const MOCK_REPOS: RepoListItem[] = [
    { name: 'Giterra-Core', stars: 120, language: 'Python' },
...
const RepositoryList = ({
    ...
    const fetchRealRepos = useCallback(async () => {
        setIsLoading(true);
        try {
            const res = await fetch(`${API_BASE_URL}/repos/${username}`);
            if (!res.ok) throw new Error('Failed');
            const data = (await res.json()) as RepoListItem[];
            setRepos(data);
        } catch (e) {
    ...
    }, [fetchRealRepos, isMockMode, username]);
...
interface DropDetectorProps {
    isDragging: boolean;
    mousePosRef: React.RefObject<{ x: number; y: number }>;
    segmentsGroupRef: React.RefObject<THREE.Group | null>;
    hoveredRef: React.MutableRefObject<number | null>;
    setHoveredSegmentState: (segmentIndex: number | null) => void;
}

const DropDetector = ({ isDragging, mousePosRef, segmentsGroupRef, hoveredRef, setHoveredSegmentState }: DropDetectorProps) => {
...
interface InteractionSegmentsProps {
    segmentsGroupRef: React.RefObject<THREE.Group | null>;
    onClick: (segmentIndex: number) => void;
}

const InteractionSegments = ({ segmentsGroupRef, onClick }: InteractionSegmentsProps) => {
    const radius = 100;
    return (
        <group ref={segmentsGroupRef}>
            {Array.from({ length: 8 }).map((_, i) => {
                ...
                return (
                    <mesh key={i} userData={{ index: i }} onClick={(e: ThreeEvent<MouseEvent>) => { e.stopPropagation(); onClick(i); }}>
...
const PlanetPage = () => {
    ...
    const [viewMode, setViewMode] = useState<'galaxy' | 'legacy'>('legacy');
    ...
    const createMockCommits = (seedName: string) => {
        ...
        return Array.from({ length: count }, (_, i) => ({
            ...
            type: COMMIT_TYPES[Math.floor(Math.random() * COMMIT_TYPES.length)],
    ...
    const fetchRealCommits = async (repoName: string) => {
        ...
        return data.map((item): CommitData => ({
            id: item.sha,
        ...
        <div className="relative h-screen w-screen bg-black overflow-hidden">
            ...
            >
                ...
                <div className="absolute bottom-8 right-8 flex gap-4 z-30">
                    <button
                        onClick={() => setViewMode((mode) => mode === 'galaxy' ? 'legacy' : 'galaxy')}
                        className="bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-100 px-6 py-3 rounded-xl backdrop-blur-md border border-indigo-300/20 font-bold"
                    >
                        {viewMode === 'galaxy' ? "Legacy 3D" : "Galaxy Orbit"}
                    </button>
                    <button
